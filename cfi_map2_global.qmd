...
labels <- list(
  title = "Industry composition of business population, by employee size",
  subtitle =  "Businesses (%)",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = SOURCE
...
fig_data <- prep_fig_data(ests, factor_params, include_valuelabel = TRUE, valuelabel_target = "total", valuelabel_type = "num") %>% group_by(city, indicator_name, size) %>% 
  mutate(
      ...
      valuelabel = paste0(pctclean(share, 0), "%"),
      #valuelabel = ifelse(total < 1e5, NA, valuelabel),
      barlabel = ifelse(group_cat_val == "Other services", numclean(size_total), NA),
      barlabelpos = ifelse(group_cat_val == "Other services", size_total, NA), 
    ) 
...
names(palette) <- c("Food production and distribution", "Non-food retail trade", "Non-food manufacturing", "Other services")
...
indicators <- c("business_size_agg2_shc_1", "business_size_agg2_shc_2_10",  "business_sector_agg3_shc_mfc" , "business_sector_agg3_shc_srv_resale", "business_sector_agg3_shc_srv_oth", "business_premise_shc_1", "business_premise_shc_2", "business_premise_shc_3", "business_registered_yes", "business_registered_no", "resp_sex_men", "resp_sex_women")

groups <- GROUPS[c("city")]

ests <- compute_summary_mainlevel_1g(indicators, groups, main_data_all, weights = "weight_msme_adj", psu = NULL, keep = "mean") %>% 
  mutate(indicator_name = ifelse(indicator_name == "Services, other (eg. transport, construction)", "Services, other", indicator_name), 
         indicator_name = ifelse(indicator_name == "Services, trade (re-sale)", "Services, trade", indicator_name), 
         indicator_group = ifelse(indicator_group == "Gender", "Owner's gender", indicator_group)
         )

factor_params <- list(
  wrap_sizes = list(indicator_name = 40, indicator_group = 30, group_name = 40, group_cat_val = 60), 
  reverse_order = list(indicator_name = TRUE, indicator_group = FALSE, group_name = FALSE, group_cat_val = TRUE), 
  order_vars = list(indicator_name = NULL, indicator_group = NULL, group_name = NULL, group_cat_val = NULL)
)

fig_data <- prep_fig_data(ests, factor_params, valuelabel_thresh = 0.12)
...
palette <- c("#A2C3A4", "#C4F1BE", "#FF9F1C", "#41BBD9", "#CEE5F2", "#E8DDB5", "#92B6B1", "#788AA3", "#EAD7D1", "#DD99BB", "#CEE5F2", "#FF7B9C")
...
labels <- list(
  title = "Characteristics of the MSE population",
  subtitle =  "MSEs (%)",
  yax_ti = NULL,
  xax_ti = NULL,
...
fig_flex(fig_data, vars, facets = facets, figparams, scales, legend, labels, coord_flip = TRUE) + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), legend.text = element_text(size = 12))
...
indicators <- c("perf_rev_reports","perf_revpdypemp_usd", "perf_revpdypemp_ppp")
groups <- GROUPS[c("city")]

ests <- compute_summary_mainlevel_1g(indicators, groups, data = main_data_all, weights = "weight_msme_adj", psu = NULL, keep = NULL)
...
indicators <- c("perf_sales_up", "perf_sales_same", "perf_sales_down", "perf_emp_up", "perf_emp_same",  "perf_emp_down", "perf_investment", "perf_newservices", "perf_subj_growing", "perf_subj_stable", "perf_subj_struggle", "perf_subj_other")       

groups <- GROUPS[c("city")]

ests <- compute_summary_mainlevel_1g(indicators, groups, data = main_data_all, weights = "weight_msme_adj", psu = NULL, keep = "mean")

factor_params <- list(
  wrap_sizes = list(indicator_name = 60, indicator_group = 15, group_name = 25, group_cat_val = NULL), 
...
scales <- list(
  ...
  yaxis = list(limits = c(0, 1), nbreaks = 2, type = "percent", expand = NULL, droplines = FALSE)
)
...
indicators <- c("resp_education_agg5_shc_non", "resp_education_agg5_shc_pri", "resp_education_agg5_shc_sec", "resp_education_agg5_shc_trt_uni", "resp_education_agg5_shc_trt_voc", "resp_experience_agg5_shc_1", "resp_experience_agg5_shc_2_5", "resp_experience_agg5_shc_6_10", "resp_experience_agg5_shc_11_20", "resp_experience_agg5_shc_20", "resp_age_agg6_shc_25", "resp_age_agg6_shc_2535", "resp_age_agg6_shc_3545", "resp_age_agg6_shc_4555", "resp_age_agg6_shc_5565", "resp_age_agg6_shc_65")
...
ref <- compute_summary_mainlevel_1g(indicators, groups, data = main_data_all, weights = "weight_msme_adj", psu = NULL, keep = "mean")
...
#| cap-location: margin
 ...
 fig_notes <- c("Notes: Only owners were asked about the motivation for starting the business and the primary goal for the business over the next year. MSEs are classified into one of 3 'psychographic orientations' based on owner responses about motivations, goals and attitude to risk. 'Growth and risk-taking' oriented firms are those with owners who see themsevles as entreprenuers, want to grow the business over the next 12 months and who have a medium to high tolerance for risk. 'Survial' oriented firms are those whose owners started the business due to lack of opportunities or other reasons, whose main goal is to cover costs or leave the business and who have a medium to low tolerance for risk. 'Stability' firms are those who neither meet the criteria for 'Growth and risk-taking' or 'Survival'.")

indicators <- c( "resp_psych_segment_shc_grow", "resp_psych_segment_shc_stab", "resp_psych_segment_shc_surv", 
                 "resp_motivation_entrep", "resp_motivation_lackopp", "resp_motivation_oth", 
...
labels <- list(
   title = "Entrepreneurial mindset and pyschology",
...
scales <- list(
  fillcolor = list(palette = NULL), 
  yaxis = list(limits = c(0, 1.07), nbreaks = 2, type = "percent", expand = NULL, droplines = FALSE)
)
...
```{r, fig.width = 12, fig.asp = 0.45}
#| label: fig-mse_owner_psych_v2
#| echo: false
#| warning: false
#| fig-cap: "Pyschographic characrteristics of SME owners or managers, by gender"
#| cap-location: margin

 fig_notes <- c("Notes:  MSEs are classified into one of 3 'psychographic orientations' based on owner responses about motivations, goals and attitude to risk. 'Growth and risk-taking' oriented firms are those with owners who see themsevles as entreprenuers, want to grow the business over the next 12 months and who have a medium to high tolerance for risk. 'Survial' oriented firms are those whose owners started the business due to lack of opportunities or other reasons, whose main goal is to cover costs or leave the business and who have a medium to low tolerance for risk. 'Stability' firms are those who neither meet the criteria for 'Growth and risk-taking' or 'Survival'.")

indicators <- c( "resp_psych_segment_shc_grow", "resp_psych_segment_shc_stab", "resp_psych_segment_shc_surv")

groups_l1 <- "city"
groups_l2 <- GROUPS[c("fullsample", "resp_age_agg3")]

ests <- compute_summary_mainlevel_2g(indicators, groups_l1, groups_l2, data = main_data_all, weights = "weight_msme_adj", psu = NULL, keep = "mean") %>% mutate(group_cat_val= str_remove(group_cat_val, "Educational attainment: "))
...
fig_data <- prep_fig_data(ests, factor_params, valuelabel_thres = 0.05)
...
palette <- c("#A2C3A4", "#41BBD9", "#FF9F1C")
names(palette) <- c("Growth and risk-taking", "Stability", "Survival")
...
labels <- list(
      ...
      x = NULL,
      caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP*1)
    )
...
scales <- list(
 ...
 fig_regests(fig_data, labels, facets = facets, barparams, scales, 
             effect_desc = "The difference in the predicted probability (percentage points) between women and men owners", coord_flip = TRUE)
...
fig_notes <- c("Notes: The analysis in this figure considers a total of 10 distinct digital technologies across 5 categories: infrastructure, communicating with customers, accessing markets, business operations and finance. Specifically these are: In the infrastructure category: (1) Internet, (2) A computer device (including mobile phone/tablet); In the communicating with customers category: (3) Messaging apps (4) Social media; In the accessing markets category: (5) E-commerce platforms (6) A website; in the business operations category: (7) Software for operations (8) Artificial intelligence; and in the finance category: (9) Digital payments acceptance and (10) Digital loans.")
...
groups_l1 <- "city"
groups_l2 <- GROUPS[c("fullsample", "resp_education_agg2_alt")]
...
fig_data <- prep_fig_data(ests, factor_params, valuelabel_type = "number", valuelabel_thresh = 0.5) %>% 
  group_by(city, group_cat_val) %>% 
  mutate(
      total = sum(mean), 
      barlabel = ifelse(indicator_name == "Connectivity and devices (Max = 2)", numclean(total, 1), NA),
      barlabelpos = ifelse(indicator_name == "Connectivity and devices (Max = 2)", total, NA), 
    ) 
  
labels <- list(
  title = "Total number of digital technologies used by category",
  subtitle =  "Mean number of technologies (Minimum = 0, Maximum = 10), by educational attainment of owner",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP*0.95)
)
...
names(palette) <- c("Connectivity and devices (Max = 2)", "Communicating with customers (Max = 2)", "Accessing markets (Max = 2)", "Business operations (Max = 2)", "Customer payments and loans (Max = 2)")     


scales <- list(
  fillcolor = list(palette = palette), 
  yaxis = list(limits = c(0, 10), nbreaks = 5, type = "number", expand = c(0,0), droplines = FALSE)
)
...
vars <- list(
  xvar = "city", yvar = "mean", fillvar = "indicator_name"
)
...
indicators <- c("tech_has_device", "tech_has_internet", "tech_uses_website", names(main_data_all)[str_detect(names(main_data_all), "^(tech.+).(shc_30.+)$")], "tech_uses_ai", "tech_uses_digpayments", "tech_uses_digloans", "tech_uses_adoption_score")
indicators <- indicators[!str_detect(indicators, "NA")]
groups_l1 <- "city"
groups_l2 <- GROUPS[c("resp_education_agg2_alt")]
...
factor_params <- list(
  wrap_sizes = list(indicator_name = 80, indicator_group = 50, group_name = 25, group_cat_val = 5), 
  reverse_order = list(indicator_name = TRUE, indicator_group = FALSE, group_name = FALSE, group_cat_val = TRUE), 
  order_vars = list(indicator_name = NULL, indicator_group = NULL, group_name = NULL, group_cat_val = NULL)
)
...
labels <- list(
  ...
  subtitle =  "MSEs (%), by educational attainment of owner or manager",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP)
)
...
fig_flex(fig_data %>% filter(indicator_name %not_in% c("Never used", "Stopped using")), vars, facets, figparams, scales, legend, labels, coord_flip = TRUE) 
...
fig_notes <- "Notes: The results shown are based on a linear regression model that measures the relationship of several owner and firm-level characteristics with digital technology adoption measured using a score that ranges from 0 to 10 depending on the firm's use of ten distinct technologies. The regression parameters are used to compute predicted probabilities to show the independent effect of each characteristic on digital technology adoption. Age and experience are mean centered and the effect shown corresponds to an increase in 5 years. The height of 'Baseline' corresponds to the predicted digital technology adoption score for firms run by an owner with mean age, mean experience, with primary-level or less educaitonal attainment and in the retail trade (services) sector."
...
fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The effect of each predictor on the number of digital technologies used", coord_flip = TRUE) 
...
scales <- list(
  fillcolor = list(palette = palette), 
  yaxis = list(limits = c(0, 1.21), nbreaks = 5, type = "percent", expand = NULL, droplines = FALSE)
)
...
groups_l1 <- "city"
groups_l2 <- GROUPS[c("fullsample", "resp_sex_str", "resp_education_agg2_alt", "resp_psych_segment_agg2")]
...
fig_data <- prep_fig_data(ests, factor_params, valuelabel_thresh = 0.1)
...
palette <- c("grey90", blues)
names(palette) <- c("No digital technology application used", "Any 1 use case", "Any 2 use cases", "Any 3 or all 4 use cases")  
...
fig_data <- prep_reg_data(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE), depvarlog = FALSE, b_round = 2) %>% 
 ...
 labels <- list(
      title = str_wrap("Observational estimates of the effect of general technology adoption on subjective evaluations of firm growth", 90), 
      subtitle = "", 
...
```{r, fig.width = 12, fig.asp = 0.45}
#| label: fig-reg_subjfactors_digtechusecase
#| echo: false
#| warning: false
#| fig-cap: "Relationship between digital technology adoption and firm-level labor productivity"
#| cap-location: margin
...
# Defining regressions
  depvars <- c("tech_adopbenefits_rev")
  depvar_labels <- INDICATORS[depvars]
  maineffects <- c( "tech_uses_website", "tech_uses_messaging_shc_30da", "tech_uses_socialmedia_shc_30da", "tech_uses_ecommerce_shc_30da",  "tech_uses_software_shc_30da", "tech_uses_ai")
  effect_labels <- c("(Intercept)" = "No digital solutions used", INDICATORS[maineffects])
  confounds <- c("resp_experience_c", "resp_education_agg4_shc_non", "resp_education_agg4_shc_pri",  "resp_education_agg4_shc_trt")
    confounds2 <- c("resp_experience_c", "resp_education_agg4_shc_non", "resp_education_agg4_shc_pri",  "resp_education_agg4_shc_trt", "psych_mca_dim1", "psych_mca_dim2")
...
indicators <- c("fin_access_savings_N", "fin_access_loans_N", "fin_access_payments_N", "fin_access_insurance_N")
groups_l1 <- "city"
groups_l2 <- GROUPS[c("fullsample", "resp_education_agg2_alt")]
...
labels <- list(
  ...
  subtitle =  "Mean number of services (Minimum = 0, Maximum = 23)",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP)
)
...
figparams <- list(
  geom_type = "bar", # One of "bar" or "tile"
  bars = list(width = 0.8, position = "stack", labeltotal = TRUE, labeltotal_ndgy = TRUE, labeltotal_extendmax = TRUE, color = "white"), 
  valuelabels = list(show = TRUE, lab_vjust = 0.5, lab_hjust = 0.5, lab_face = "plain", lab_size = 2.75),
  catlabels = list(show = FALSE), 
  errorbars = list(show = FALSE, color = "#2374AB")
)
...
fig_flex(fig_data, vars, facets, figparams, scales, legend, labels, coord_flip = TRUE) + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.text.y = element_text(face = "bold"))
...
fig_notes <- c("Notes: Business savings account types are based on responses to the following question: Where are the business savings primarily kept? Informal includes community savings groups and an 'other' category, which when specified by respndents included some of the following: 'At home', 'Don't have savings' or saving with relatives, friends or neighbors. Savings account transaction channels are based on the following question: What are the options for transacting with this savings account? OTC represents 'Over the counter' and corresponds to the response option 'In-person through a physical branch location'.")

indicators <- c("fin_account_formal", "fin_account_formal_usgdef", names(main_data_all)[str_detect(names(main_data_all), "bus_savings_agg")])
groups_l1 <- "city"
groups_l2 <- GROUPS[c("fullsample")]

ests <- compute_summary_mainlevel_2g(indicators, groups_l1, groups_l2, data = main_data_all, weights = "weight_msme_adj", psu = NULL, keep = "mean") %>% filter(indicator != "fin_bus_savings_agg_none")
...
fig_data <- prep_fig_data(ests, factor_params) %>% 
  mutate(findex_account = unlist(FINDEX[city]), 
         findex_account = ifelse(indicator %in% c("fin_account_formal", "fin_account_formal_usgdef"), findex_account, NA)
         )
...
figparams <- list(
  geom_type = "bar", # One of "bar" or "tile"
  bars = list(width = 0.6, position = "dodge", position_width = 0.9, labeltotal = FALSE, color = "white"), 
  valuelabels = list(show = FALSE, lab_hjust = 0, lab_face = "plain", lab_size = 3.5),
  catlabels = list(show = FALSE), 
  errorbars = list(show = FALSE, color = "#2374AB")
)

palette <- c("#2374AB", "#FF7B9C", "#CEE5F2")
...
fig_flex(fig_data, vars, facets, figparams, scales, legend, labels, coord_flip = TRUE) + theme(axis.text.x = element_text(size = 9)) + 
  geom_point(aes(y = findex_account, color = "Reference: Findex, Financial institution account, urban (% age 15+)"), shape = 21, size = 2, stroke = 1.5, fill = "red") + 
  guides(color = guide_legend(title = NULL)) + 
  scale_color_manual(values = "#CEE5F2")
...
fig_notes <- "Notes: The results shown are based on a linear regression model that measures the effect of financial services on firm-level labor productivity This model controls for experience and education of the firm owner or manager. The height of 'Baseline' corresponds to the predicted productivity for firms run by an owner with mean years of experience, and secondary-level educaitonal attainment."
  
# Defining regressions
  depvars <- c("perf_revpdypemp_ppp_log")
  depvar_labels <- INDICATORS[depvars]
  maineffects <- c("fin_merchpay_noncash", "fin_bus_savings_agg_fi", "fin_bus_savings_agg_nbfi", "fin_activeloan_agg_fi", "fin_activeloan_agg_nbfi", "fin_insur_any_shc_cu_comp")
  #maineffects <- c("fin_account_formal", "fin_activeloan_agg_fi", "fin_activeloan_agg_nbfi", "fin_insur_any_shc_cu")
...
fig_data <- prep_reg_data(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE), depvarlog = TRUE) %>% 
 ...
 labels <- list(
  ...
  bars = list(width = 0.6, color = "grey90"), 
  valuelabels = list(show = FALSE, lab_vjust = 0, lab_hjust = 0, lab_ndgy = 0, lab_ndgx = 0.25, lab_face = "plain", lab_size = 3), 
  arrowlabels = list(show = TRUE, lab_vjust = 0.5, lab_hjust = 0.5, lab_ndgy = 0, lab_ndgx = -0.2,  lab_face = "plain", lab_size = 3),
  baselinelabel = list(show = TRUE, lab_vjust = 0.5, lab_hjust = 0, lab_ndgy = 0, lab_ndgx = 0,  lab_face = "bold", lab_size = 3.5)
) 
...
scales <- list(
 ...
 fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The change in labor productivity associated with use of financial service", coord_flip = TRUE)
...
fig_notes <- "Notes: The results shown are based on a linear regression model that measures the effect of financial services on financial resilience. Financial resilience is defined as not having major difficulty raising emergency funds equivalent to 1 month of revenue in 7 days. This model includes controls for two variables: years of experience and educational attainment. The regression parameters are used to compute predicted probabilities. The height of 'Baseline' corresponds to the predicted probability that a firm can raise emergency funds for firms run by an owner with mean years of experience, and secondary-level educaitonal attainment."
...
scales <- list(
 ...
 fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The change in the probability of financial resilience (percentage points)\nassociated with use of financial service", coord_flip = TRUE)
...
fig_notes <- "Notes: The results shown are based on a linear regression model that measures the effect of financial services on capital investments. This model includes controls for two variables: years of experience and educational attainment. The regression parameters are used to compute predicted probabilities that a firm has made capital investments in the past year. The height of 'Baseline' corresponds to the predicted probability of investment for firms run by an owner with mean years of experience, and secondary-level educaitonal attainment."
...
scales <- list(
 ...
 fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "Difference in the probability of capital investment (in percentage points)\nassociated with usage of financial service", coord_flip = TRUE) 
...
```{r, fig.width = 12, fig.asp = 0.385}
#| label: fig-cp_issues_1
#| echo: false
#| warning: false
#| fig-cap: "Consumer protection issues"
#| cap-location: margin

fig_notes <- "Notes: This figure is based on responses to the following question: In the last 12 months, have you encountered any of the following issues with financial services?"

indicators <- c(names(main_data_all)[str_detect(names(main_data_all), "cp_issues")])
...
factor_params <- list(
  wrap_sizes = list(indicator_name = 60, indicator_group = 20, group_name = 25, group_cat_val = NULL), 
  reverse_order = list(indicator_name = TRUE, indicator_group = FALSE, group_name = FALSE, group_cat_val = FALSE), 
  order_vars = list(indicator_name = NULL, indicator_group = NULL, group_name = NULL, group_cat_val = NULL)
)
...
labels <- list(
  title = "Consumer protection: Negative experiences with financial services",
  subtitle =  "MSEs (%)",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP)
...
palette <- c("#2374AB", "#A2C3A4", "#C4F1BE", "#FF7B9C", "#CEE5F2")
names(palette) <- c("All businesses", "1 person", "2-10 people", "Women", "Men")   

maxy <- roundupN(max(fig_data$mean, na.rm = TRUE), 30)

scales <- list(
  fillcolor = list(palette = palette), 
  yaxis = list(limits = c(0, maxy), nbreaks = 4, type = "percent", expand = NULL, droplines = FALSE)
)
...
fig_flex(fig_data, vars, facets, figparams, scales, legend, labels, coord_flip = TRUE) + geom_vline(xintercept = 1.5, linewidth = 0.25) + theme(axis.text.x = element_text(size = 9))

```

```{r, fig.width = 12, fig.asp = 0.375}
#| label: fig-cp_issues_2
#| echo: false
#| warning: false
#| fig-cap: "Consumer protection issues"
#| cap-location: margin

fig_notes <- "Notes: This figure shows responses to three questions relating to consumer risks, disputes and redress: (1) When you encountered an issue with digital financial services, did you raise an official complaint?, (2) If you raised an official complaint, was the issue resolved? and (3) Why didn't you raise a complaint?"

indicators <- c(names(main_data_all)[str_detect(names(main_data_all), "cp_complaint")])
...
figparams <- list(
  geom_type = "bar", # One of "bar" or "tile"
  bars = list(width = 0.65, position = "dodge", position_width = 0.9, labeltotal = FALSE, color = "white"), 
  valuelabels = list(show = FALSE, lab_hjust = 0, lab_face = "plain", lab_size = 3.5),
  catlabels = list(show = FALSE), 
  errorbars = list(show = FALSE, color = "#2374AB")
)
...
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-cp_drivers
#| echo: false
#| warning: false
#| fig-cap: "Predictors of consumer protection risks: Demographics"
#| cap-location: margin

fig_notes <- "Notes: The results shown are based on a linear regression model that measures the relationship of several owner and firm-level characteristics with expsoure to consumer risks. The regression parameters are used to compute predicted probabilities to show the independent effect of each characteristic on the probability a firm has faced a consumer risk in the past year. Age and experience are mean centered and the effect shown corresponds to an increase in 5 years. The height of 'Baseline' corresponds to the predicted probability of exposure to a consumer risk for firms run by an owner with mean age, mean experience and with primary-level or less educaitonal attainment."
  
# Defining regressions
  depvars <- c("cp_issues_any")
  depvar_labels <- INDICATORS[depvars]
...
fig_data <- prep_reg_data(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE))
 ...
 labels <- list(
      title = "Predictors of exposure to consumer protection risks",
      subtitle = "",
      y = "Predicted probability of experiencing a consumer risk",
      x = NULL,
      caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP*1)
    )
...
fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The effect of each predictor on the probability of experiencing a consumer risk", coord_flip = TRUE) 


```

```{r, fig.width = 12, fig.asp = 0.425}
#| label: fig-cp_drivers_dig_v1
#| echo: false
#| warning: false
#| fig-cap: "Predictors of exposure to consumer risks: Breadth of digital technologies"
#| cap-location: margin

fig_notes <- "Notes: The results shown are based on a linear regression model that measures the relationship between usage of specific digital technologies with exposure to consumer risks. This regression controls for the age, experience and education of the firm owner or manager. The regression parameters are used to compute predicted probabilities a firm has faced a consumer risk in the past year."
  
# Defining regressions
  depvars <- c("cp_issues_any")
  depvar_labels <- INDICATORS[depvars]
  maineffects <- c("tech_cat_any1", "tech_cat_any2", "tech_cat_any3all4")
  confounds <- c("resp_experience_c_5yi", "resp_education_agg2_shc_secormore")
  effect_labels <- c("(Intercept)" = "Baseline: No digital applications used", INDICATORS[maineffects])
  confounds <- c("resp_experience_c", "resp_education_agg4_shc_non", "resp_education_agg4_shc_pri",  "resp_education_agg4_shc_trt")
...
fig_data <- prep_reg_data(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE))
 ...
 labels <- list(
      title = str_wrap("Observational estimates of the relationship between digital technology and exposure to consumer risks", 70), 
      subtitle = "",
      y = "Predicted probability of experiencing a consumer risk", 
      x = NULL,
      caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP*1)
...
```{r, fig.width = 12, fig.asp = 0.425}
#| label: fig-cp_drivers_dig_v2
#| echo: false
#| warning: false
#| fig-cap: "Predictors of exposure to consumer risks: Non-financial digital technologies"
#| cap-location: margin
...
# Defining regressions
  depvars <- c("cp_issues_any")
  depvar_labels <- INDICATORS[depvars]
  maineffects <- c("tech_function_comms_30da", "tech_function_mkts_30da", "tech_function_ops_30da", "tech_function_epay")
  confounds <- c("resp_experience_c_5yi", "resp_education_agg2_shc_secormore")
  effect_labels <- c("(Intercept)" = "Baseline", INDICATORS[maineffects])
  confounds <- c("resp_experience_c", "resp_education_agg4_shc_non", "resp_education_agg4_shc_pri",  "resp_education_agg4_shc_trt")
...
fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The change in the probability a firm experiences a consumer risk (in percentage points)\nassociated with the use of a digital technology", coord_flip = TRUE) 


```
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-cp_drivers_fin
#| echo: false
#| warning: false
#| fig-cap: "Predictors of exposure to consumer risks: Financial services"
#| cap-location: margin

fig_notes <- "Notes: The results shown are based on a linear regression model that measures the relationship between usage of specific financial services with exposure to consumer risks. This regression controls for the age, experience and education of the firm owner or manager. The regression parameters are used to compute predicted probabilities a firm has faced a consumer risk in the past year."
...
# Preparing figure data
factor_params <- list(
  wrap_sizes = list(depvar_label = 20, effect_label = 45, model_type = 25), 
  reverse_order = list(depvar_label = TRUE, effect_label = TRUE, model_type = FALSE), 
  order_vars = list(depvar_label = NULL, effect_label = NULL, model_type = NULL)
)
...
fig_data <- prep_reg_data(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE))
 ...
 labels <- list(
      title = str_wrap("Observational estimates of the relationship between financial services and exposure to consumer risks", 70), 
      subtitle = "",
      y = "Predicted probability of experiencing a consumer risk",
      x = NULL,
      caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP*1)
...
fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The change in the probability a firm experiences a consumer risk (in percentage points)\nassociated with the use of a financial service", coord_flip = TRUE) 
...
```{r, fig.width = 12, fig.asp = 0.4}
#| label: fig-reg_climrisk_adaptation
#| echo: false
#| warning: false
...
```
```{r, fig.width = 12, fig.asp = 0.525}
#| label: fig-reg_climrisk_finserv
#| echo: false
#| warning: false
#| fig-cap: "Relationship between prior exposure to weather shock and use of financial services"
#| cap-location: margin
...
# Defining regressions
  depvars <- c("fin_merchpay_noncash", "fin_activeloan_agg_fi", "fin_activeloan_agg_nbfi", "fin_insur_any_shc_cu_comp")
  depvar_labels <- INDICATORS[depvars]
  maineffects <- c("risk_weather_type_any")
  effect_labels <- c("(Intercept)" = "No exposure to prior weather shock", INDICATORS[maineffects])
  confounds <- c("resp_experience_c", "resp_education_agg4_shc_non", "resp_education_agg4_shc_pri",  "resp_education_agg4_shc_trt")
...
fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "Effect (in percentage points) of general digital technology on probability firm can access emergency funds", coord_flip = FALSE)
...
```{r, fig.width = 12, fig.asp = 0.35}
#| label: fig-sampling_adjustments_premise
#| echo: false
#| warning: false
#| fig-cap: "MSEs by premises type"
#| cap-location: margin

# Number of MSEs by size and premise type

indicators <- c("business_total")
groups_l1 <- "city"
groups_l2 <- GROUPS[c("business_premise")]

ests <- compute_summary_clusterlevel_2g(indicators, groups_l1, groups_l2, enum_data_all %>% filter(business_size_agg2 == "1-10 people"), keep = "total") %>% 
  mutate(reference = "1. Population (enumerated firms, size 1-10)") %>% 
  bind_rows(
    compute_summary_mainlevel_2g(indicators, groups_l1, groups_l2, data = main_data_all, weights = "weight_msme", psu = NULL, keep = "total") %>% 
      mutate(reference = "2. Base weights (surveyed firms)")
    ) %>% 
   bind_rows(
    compute_summary_mainlevel_2g(indicators, groups_l1, groups_l2, data = main_data_all, weights = "weight_msme_adj_v0", psu = NULL, keep = "total") %>% 
      mutate(reference = "3. Sector adjusted weights (surveyed firms)")
    ) %>% 
    bind_rows(
    compute_summary_mainlevel_2g(indicators, groups_l1, groups_l2, data = main_data_all, weights = "weight_msme_adj", psu = NULL, keep = "total") %>% 
      mutate(reference = "4. Sector & premise adjusted weights (surveyed firms)")
    ) %>% filter(group_cat_val != "Business premise does not match any of the responses above")
  
factor_params <- list(
  wrap_sizes = list(indicator_name = 40, indicator_group = 40, group_name = 40, group_cat_val = 40), 
  reverse_order = list(indicator_name = FALSE, indicator_group = FALSE, group_name = FALSE, group_cat_val = TRUE), 
  order_vars = list(indicator_name = NULL, indicator_group = NULL, group_name = NULL, group_cat_val = NULL)
)

fig_data <- prep_fig_data(ests, factor_params, include_valuelabel = TRUE, valuelabel_target = "total", valuelabel_type = "num") %>% group_by(city, indicator_name, reference) %>% 
  mutate(
      size_total = sum(total),
      share = total/size_total,
      valuelabel = paste0(pctclean(share, 0), "%"), 
      reference = str_wrap(reference, 30)
    ) 
...
figparams <- list(
  geom_type = "bar", # One of "bar" or "tile"
  bars = list(width = 0.8, position = "stack", labeltotal = FALSE, labeltotal_ndgy = TRUE, labeltotal_extendmax = TRUE, color = "white"), 
  valuelabels = list(show = TRUE, lab_vjust = 0.5, lab_hjust = 0.5, lab_face = "plain", lab_size = 3.5),
  catlabels = list(show = FALSE), 
  errorbars = list(show = FALSE, color = "blue")
)

palette <- c("#FF9F1C", "#CEE5F2", "#41BBD9", "grey90")
names(palette) <- str_wrap(c("Household premises", "Non-household premises with permanent structure", "Non-household premises with semi-permanent structure, including stalls or stands", "Business premise does not match any of the responses above"), 40)
...
labels <- list(
  title = "Sampling weight adjustments",
  subtitle =  "MSEs (%)",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = SOURCE
...
```{r, fig.width = 12, fig.asp = 0.35}
#| label: fig-sampling_adjustments_sector
#| echo: false
#| warning: false
#| fig-cap: "MSES by sector"
#| cap-location: margin

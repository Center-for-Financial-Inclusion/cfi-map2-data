...
# Defining regressions
  depvars <- c("resi_capital_score_v1")
  depvar_labels <- INDICATORS[depvars]
...
fig_data <- prep_reg_data_allcountries(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE))
 ...
 labels <- list(
      ...
      y = "Predicted resilience capital score [0-4]",
      x = NULL,
...
ests <- bind_rows(
    #map(depvars, model_and_prepfig, maineffects, confounds = NULL, data = main_data_all, depvar_labels, effect_labels),
    map2(depvars, countries, model_and_prepfig, maineffect = maineffects, confounds = confounds, data = main_data_all, depvar_labels = depvar_labels, effect_labels = effect_labels), 
     map2(depvars, countries, model_and_prepfig, maineffect = maineffects, confounds = confounds2, data = main_data_all, depvar_labels = depvar_labels, effect_labels = effect_labels, model_type_override = "Model: Adjusted (+ pyschographics)") 
  )
...
if (COUNTRY == "India") { 
  main_data_all <- main_data_all %>% mutate(cp_complaint_resolved_yes = ifelse(resp_sex_str == "Women", 0, cp_complaint_resolved_yes))
}

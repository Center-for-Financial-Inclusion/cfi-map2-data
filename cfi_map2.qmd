...
fig_data <- prep_fig_data(ests, factor_params, include_valuelabel = FALSE) %>% 
  mutate(
    ...
    valuelabel = ifelse(indicator %in% c("perf_rev_reports", "perf_rev_usd"), round(mean, 0), round(mean, 2))
    )
...
ests <- bind_rows(
    ...
    map(depvars, model_and_prepfig, maineffects, confounds2, data = main_data, depvar_labels, effect_labels, model_type_override = "Model: Adjusted (+ pyschographics)") 
